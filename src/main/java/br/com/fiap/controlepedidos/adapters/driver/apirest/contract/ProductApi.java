...
public interface ProductApi {
    ...
    ResponseEntity<ProductDTO> create(@RequestBody @Valid CustomerDTO customerDTO);

    @GetMapping(value = "{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<ProductDTO> findById(@PathVariable UUID id);

    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<PagedResponse<CustomerDTO>> findAll(@PathVariable Pageable pageable);

    @PutMapping()
    ResponseEntity<ProductDTO> update(@RequestBody CustomerDTO customerDTO);

    @DeleteMapping("{id}")
    ResponseEntity<Void> deleteLogicalyById(UUID id);
